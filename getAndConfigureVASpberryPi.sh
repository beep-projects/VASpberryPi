...
if [[ -d "VASpberryPi" ]]; then
  rm -rf "VASpberryPi"
fi

git clone https://github.com/beep-projects/VASpberryPi
cd VASpberryPi || error "git clone failed"
...
sed -i "s/^ADMIN_DEFAULT_PASSWORD=.*/ADMIN_DEFAULT_PASSWORD=${PASSWORD}/" ${FILES_FOR_PI_FOLDER}/secondrun.sh
...
USE_LATEST=false
while true; do
    read -rp "Do you want to use latset Raspberry Pi OS (y, Y, yes) or use the last tested one (n, N, no)? " yn
    case $yn in
...
sed -i "s/^USE_LATEST_RASPI_OS=.*/USE_LATEST_RASPI_OS=${USE_LATEST}/" ./install_vaspberrypi.sh

./install_vaspberrypi.sh